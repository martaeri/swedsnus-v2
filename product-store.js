(() => {
  ...
  function media(row, className, eager = false, overlay = '') {
    ...
    if (!src) return `<div class="${className}"><span class="product-image-placeholder">Produktbild</span>${overlay}</div>`;
    return `<div class="${className} has-product-image"><img src="${escapeHtml(src)}" alt="${label}" ${eager ? 'fetchpriority="high"' : 'loading="lazy"'} data-product-image><span class="product-image-placeholder" hidden>Produktbild</span>${overlay}</div>`;
  }
  ...
  function variantGroup(row) {
    if (row.accessory_type === 'Metalldosa') return 'tillbehor:metalldosa';
    if (row.accessory_type === 'Tomdosa') return `tillbehor:tomdosa:${String(row.compatible_with || '').toLowerCase()}`;
    return row.product_id;
  }
  function group(row) { const groupKey=variantGroup(row);return state.rows.filter(candidate => variantGroup(candidate) === groupKey); }
  ...
  function card(row,options={}) {
    ...
    return `<article class="product-card" data-product-id="${id}" data-category="${category(row)}" data-name="${escapeHtml(name(row).toLowerCase())}" data-taste="${escapeHtml(String(row.taste_variables || row.taste_display || '').toLowerCase())}" data-type="${escapeHtml(String(row.accessory_type || row.product_line || row.aroma_type || '').toLowerCase())}" data-series="${escapeHtml(String(row.compatible_with || row.product_line || row.aroma_type || '').toLowerCase())}" data-material="${escapeHtml(String(row.material || '').toLowerCase())}" data-color="${escapeHtml(String(row.filter_color || '').toLowerCase())}" data-design="${escapeHtml(String(row.design_color || '').toLowerCase())}" data-format="${escapeHtml(String(row.format || row.grind || '').toLowerCase())}" data-strength="${escapeHtml(String(row.strength || '').toLowerCase())}"><button class="bookmark" type="button" data-bookmark="${id}" aria-label="Spara produkt" aria-pressed="false">${BOOKMARK_ICON}</button><a class="product-card-main" href="${url(row)}">${media(row,'product-image',false,strengthBadge(row.strength))}<span class="product-tag">${escapeHtml(row.product_line || row.aroma_type || row.accessory_type || row.product_family)}</span><h3 class="product-name">${escapeHtml(name(row))}</h3>${row.taste_display ? `<p class="product-meta">Smak: ${escapeHtml(row.taste_display)}</p>` : ''}${row.format ? `<p class="product-meta">Format: ${escapeHtml(row.format)}</p>` : ''}</a>${subscriptionOnly ? '' : `<div class="pack-control"><label>Flerpack</label>${packMenu(row)}</div>`}${!subscriptionOnly&&subscriptionEligible(row)?`<button class="subscription-reveal" type="button" data-subscription-open="${id}">Prenumerera på produkten</button>`:''}<div class="product-card-bottom${subscriptionOnly?' subscription-product-bottom':''}"><div class="product-price"><span data-selected-total>${money(firstPack.total)}</span><small data-selected-per-dose>${firstPack.perDose.toLocaleString('sv-SE',{minimumFractionDigits:2,maximumFractionDigits:2})} kr/${row.amount_dosor ? 'dosa' : 'st'}</small></div><button class="${subscriptionOnly?'subscribe-card-add':'add-cart'}" type="button" ${subscriptionOnly?`data-subscription-open="${id}"`:`data-add-cart="${id}"`}>${subscriptionOnly?'Prenumerera':'Lägg i varukorg'}</button></div></article>`;
  }
