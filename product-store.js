(() => {
  ...
  async function load() {
    ...
    state.ready = true;
    window.SwedsnusV2 = { state, visible, escapeHtml, slugify, name, key, url, money, price, category, image, media, packs, packMenu, purchaseChoice, subscriptionEligible, group, card, find: id => state.rows.find(row => key(row) === id || row.product_id === id || row.variant_id === id) };
