(() => {
  ...
  const allowed=new Set(['swedsnus-blue','cool-blue','cool','warm','green','cool-terracotta','cool-ochre','cool-forest']);
  const saved=localStorage.getItem(KEY);
  const initial=allowed.has(saved)?saved:'swedsnus-blue';
  document.documentElement.dataset.theme=initial;
  function apply(value) {
    const theme=allowed.has(value)?value:'swedsnus-blue';
